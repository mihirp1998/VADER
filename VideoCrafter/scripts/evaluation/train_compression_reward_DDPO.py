...
def run_training(args, gpu_num, gpu_no, **kwargs):
    ...
    peft_model = DiffusionPipeline.from_pretrained("kvablack/ddpo-compressibility").to(accelerator.device)
        ...
        for idx, step in enumerate(range(default_iterations)): # default number of iterations is 100
            ...
            with accelerator.accumulate(scorer):    # gradient accumulation
                with accelerator.autocast():            # mixed precision
                    ...
                    # generate images
                    with torch.no_grad():
                        images_output = peft_model(prompts).images

                    batch_samples = np.array([np.array(temp_im) for temp_im in images_output]) # BHWC
                    # reshape to BCHW
                    batch_samples = batch_samples.transpose(0, 3, 1, 2) # BHWC -> BCHW
                    video_frames_ = torch.from_numpy(batch_samples).to(accelerator.device, dtype=scorer.module.dtype)
                    
                    # start training the reward model
                    images = (video_frames_/255.0).clamp(0, 1)    # images are in the range of [0, 1]
                ...
                if global_step >= args.max_train_steps:
                    break
            # free memory
            del batch_samples, video_frames_, images_output
